...
void ZvejysServer::RegisterRoute(HttpMethod httpMethod, const std::string& path, const HttpHandler& handler) {
    ...
    if (path.find('{') != std::string::npos) {
        // Pattern route — store separately
        pattern_routes_.emplace_back(path, std::move(handlerNode));
    } else {
        // Exact route — fast map lookup
        route_map_.insert({path, std::move(handlerNode)});
    }
}
...
void ZvejysServer::RegisterAuthenticatedRoute(HttpMethod httpMethod,const std::string &path, AuthenticatedHttpHandler handler) {
    ...
    if (path.find('{') != std::string::npos) {
        // Pattern route — store separately
        pattern_routes_.emplace_back(path, std::move(authenticatedRouteNode));
    } else {
        // Exact route — fast map lookup
        route_map_.insert({path, std::move(authenticatedRouteNode)});
    }
    // route_map_.insert(std::pair<const std::string, std::shared_ptr<PathTreeNode>>(path, std::move(authenticatedRouteNode)));
}
...
int ZvejysServer::HandleEpollNewConnection(int epollFD, epoll_event event) {
    ...
    auto conn = std::make_shared<HttpConnection>(client_socket_fd, this, epollFD);
...
void ZvejysServer::DebugHttpRoutes()
{
    std::cout << "I have HTTP routes: " << GetRouteMap().size() + GetPatternRoutes().size() << std::endl;
    std::cout << "Routes: " << std::endl;
    for (auto route = GetRouteMap().begin(); route != GetRouteMap().end(); ++route) {
        std::cout << route->first << std::endl;
    }
    for (auto route = GetPatternRoutes().begin(); route != GetPatternRoutes().end(); ++route)
    {
        std::cout << route->first << std::endl;
    }
}
