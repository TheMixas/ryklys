...
void HttpConnection::HandleRead(HttpRequest request)
{
    ...
    if (isWsUpgrade)
    {
        ...
        GetServer()->UpgradeToWebSocket(GetSocketFD(), epollFD, std::move(request));
    }
    ...
    else
    {
        ...
        else
        {
            // Try pattern routes
            bool matched = false;
            for (auto& [pattern, node] : GetServer()->GetPatternRoutes())
            {
                std::unordered_map<std::string, std::string> params;
                if (ZvejysServer::MatchPattern(pattern, request.path, params)
                    && node->GetRouteMethod() == request.method)
                {
