...
#include "./../test-route/TestRoutes.h"
...
void RegisterTestRoutes(ZvejysServer &server) {
    ...
    server.RegisterWebSocketRoute("/ws", [](std::shared_ptr<WebSocketConnection> ws) {
        ws->SetOnMessage([](std::shared_ptr<WebSocketConnection> ws_,
            ...
            if (opcode == WsOpcode::TEXT) {
                ...
                ws_->SendText("Echo: " + msg);
            } else {
                ws_->SendBinary(data);
            }
        });

        ws->SetOnClose([](std::shared_ptr<WebSocketConnection> ws_,
                         uint16_t code,
    ...
    server.RegisterWebSocketRoute("/ws2", [](std::shared_ptr<WebSocketConnection> ws) {
        ws->SetOnMessage([](std::shared_ptr<WebSocketConnection> ws_,
                           const std::vector<uint8_t> &data,
                           WsOpcode opcode) {
            // Echo back whatever we receive
